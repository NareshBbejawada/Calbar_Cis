...
<#
.SYNOPSIS
    Read-only Windows volume capacity inventory with an HTML email report.
.DESCRIPTION
    Reads Windows SQL-host names from servers.txt; queries Win32_Volume and
    Win32_MountPoint. Collection remains read-only. Email is queued in msdb
    using an EXISTING SQL Server Database Mail profile. No configuration
    changes, cleanup, SQL/service restarts, or reboots are performed.
    Auto transport: WSMan first, then DCOM if the volume query fails.
    PowerShell does not connect to SMTP. Windows authentication is used to
    connect to the mail-enabled SQL instance. Its profile supplies the sender.
    SQL connections are encrypted and validate the SQL Server certificate.
    Report files are always written before an email is attempted.
.EXAMPLE
    .\Send-SQLDriveCapacityReport-DatabaseMail.ps1 -NoEmail -OpenReport
.EXAMPLE
    .\Send-SQLDriveCapacityReport-DatabaseMail.ps1
.EXAMPLE
    .\Send-SQLDriveCapacityReport-DatabaseMail.ps1 -OnlySendOnAlert -ConnectionProtocol DCOM
.NOTES
    Run with Windows PowerShell 5.1 (powershell.exe) on Windows.
    Uses built-in .NET System.Data.SqlClient; no SqlServer module is required.
    TestEmailOnly sends ONE test message, without collecting drives.
    CSV attachment mode requires execution on the mail SQL instance's host.
    Successful submission means QUEUED, not confirmed mailbox delivery.
    Exit codes: 0 = completed; 1 = configuration/report/email failure;
                2 = report completed with failed/partial collection.
    A low-space alert alone does not produce a nonzero exit code.
    Review and pilot-test on a small server list before scheduling.
#>
[CmdletBinding()]
param(
